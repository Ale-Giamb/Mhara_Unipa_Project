...
PORT = 8080
ROOT_DIR = "/home/roboticslab/Scrivania/mhara_env/MHARA_Unipa/src/qi_unipa/html_pages"
...
def get_local_ip_tablet():
    """Ottiene l'indirizzo IP locale della macchina (192.168.x.x o 10.x.x.x)"""
    try:
        # Metodo 1: Usa il comando 'ip route' per trovare l'IP della rete locale
        result = subprocess.run(['ip', 'route'], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            # Cerca la linea con 'default via' per trovare l'interfaccia principale
            for line in result.stdout.split('\n'):
                if 'default via' in line:
                    interface_match = re.search(r'dev (\w+)', line)
                    if interface_match:
                        interface = interface_match.group(1)
                        # Ottieni l'IP di questa interfaccia
                        ip_result = subprocess.run(['ip', 'addr', 'show', interface], 
                                                 capture_output=True, text=True, timeout=5)
                        if ip_result.returncode == 0:
                            # Cerca l'IP nell'output
                            ip_match = re.search(r'inet (192\.168\.\d+\.\d+|10\.\d+\.\d+\.\d+)', 
                                               ip_result.stdout)
                            if ip_match:
                                return ip_match.group(1)
    except Exception as e:
        logger.error(f"Errore metodo 1: {e}")
    
    try:
        # Metodo 2: Usa hostname -I (se disponibile)
        ...
        logger.error(f"Errore metodo 2: {e}")
...
class ImprovedHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            path_handlers = {
                ...
                "mostra_immagine.html": self._handle_test_page,
                "thinking.html": self._handle_simple_page
            }

            if self.path.strip("/") == "stream":
                self._handle_mjpeg_stream()
                return


            # --- Nuovo blocco per servire immagini ---
            if path.endswith(('.jpg', '.jpeg', '.png', '.gif')):
                file_path = os.path.join(ROOT_DIR, path)  # ROOT_DIR è la cartella html_pages
                if os.path.exists(file_path):
                    self._serve_image(file_path, client_addr=client_addr)
                else:
                    logger.error(f"[{client_addr}] File immagine non trovato: {file_path}")
                    self.send_error(404, "File non trovato")
                return  # esce subito perché l'immagine è già servita
    ...
    def _serve_image(self, file_path, client_addr="unknown"):
        try:
            ...
            #logger.info(f"[{client_addr}] Immagine servita: {os.path.basename(file_path)}")
        except Exception as e:
            logger.error(f"[{client_addr}] Errore nel servire immagine: {e}")
            try:
                self.send_error(500, "Errore nel servire immagine")
            except:
                pass
    
    def _handle_mjpeg_stream(self):
        """Invia uno stream MJPEG continuo"""
        self.send_response(200)
        ...
        try:
            while True:
                # Legge l'ultima immagine salvata
                file_path = os.path.join(ROOT_DIR, 'img', 'ultima.jpg')
            ...
            #logger.info("Connessione MJPEG chiusa dal client.")
            pass
        except Exception as e:
            logger.error(f"Errore nello stream MJPEG: {e}")
...
class qi_unipa_server(Node):
    def __init__(self):
        super().__init__('qi_unipa_server')
    ...
    def _create_publishers(self):
        ...
        self.server_url_tablet_pub = self.create_publisher(String, '/ip_tablet', 10)
    ...
    def publish_server_url(self):
        ...
        try:
            ...
            local_ip = get_local_ip_tablet()
            server_url = f"http://{local_ip}:{PORT}"
            
            msg = String()
            msg.data = server_url
            self.server_url_tablet_pub.publish(msg)
    ...
    def check_requests_user(self):
        ...
        while not request_queue_user.empty() and processed_count < max_process_per_cycle:
            try:
                ...
                if (nome or cognome or peso) and self.flag :  # Validazione base
                    self.flag=False
                    msg = String()
