...
class Go2Controller:
    ...
    async def speak(self, text):
        ...
        if proc.returncode == 0:
            ...
            LOGGER.error("Errore generazione TTS Piper")
...
class Go2RobotNode(Node):
    ...
    def action_callback(self, msg: String):
        """Riceve comandi azione: stand_up, stand_down, stop"""
        if global_controller is None:
        ...
        try:
            ...
            if action == "stand_up":
                asyncio.run_coroutine_threadsafe(global_controller.stand_up(), loop)
            elif action == "stand_down":
                asyncio.run_coroutine_threadsafe(global_controller.stand_down(), loop)
            elif action == "stop":
                asyncio.run_coroutine_threadsafe(global_controller.stop(), loop)
            else:
                self.get_logger().warn(f"Azione sconosciuta: {action}")
        except Exception as e:
    ...
    def speak_callback(self, msg: String):
        ...
        if global_controller is None:
            self.get_logger().debug("Go2 non disponibile, ignorando speak")
            return
            
        try:
            text = msg.data.strip()
            self.get_logger().debug(f"Speak: {text}")
